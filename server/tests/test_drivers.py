...
from tft_artemis.drivers import vm_info_to_ip
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({'address': '127.0.0'}, 'address', None, Error(tft_artemis.Failure('failed to parse an IP address'))),
    ],
)
def test_vm_info_to_ip(
    output: tft_artemis.JSONType, key: str, regex: Optional[Pattern[str]], expected: Result[str, tft_artemis.Failure]
) -> None:
    ...
    if expected.is_ok:
        ...
        assert isinstance(r_ip.unwrap_error(), tft_artemis.Failure)


def test_driver_instantiate(logger: gluetool.log.ContextAdapter) -> None:
    r_pool = tft_artemis.drivers.PoolDriver._instantiate(logger, 'localhost', 'dummy-localhost', {})

    assert r_pool.is_ok

    pool = r_pool.unwrap()

    assert isinstance(pool, tft_artemis.drivers.localhost.LocalhostDriver)


def test_driver_instantiate_unknown_driver(logger: gluetool.log.ContextAdapter) -> None:
    r_pool = tft_artemis.drivers.PoolDriver._instantiate(logger, 'unknown-driver', 'dummy-pool', {})
...
def test_driver_instantiate_failed_sanity(logger: gluetool.log.ContextAdapter, mockpatch: MockPatcher) -> None:
    mockpatch(tft_artemis.drivers.localhost.LocalhostDriver, 'sanity').return_value = Error(
        tft_artemis.Failure('failed sanity')
    )
...
def test_driver_load_or_none(
    ...
    mockpatch(tft_artemis.drivers.SafeQuery, 'one_or_none').return_value = Ok(mock_record)  # type: ignore[attr-defined]
    mockpatch(tft_artemis.drivers.PoolDriver, '_instantiate').return_value = Ok(mock_pool)

    r_pool = tft_artemis.drivers.PoolDriver.load_or_none(logger, session, 'dummy-pool')

    assert r_pool.is_ok

    pool = r_pool.unwrap()

    cast(MagicMock, tft_artemis.drivers.PoolDriver._instantiate).assert_called_once_with(
...
def test_driver_load_or_none_no_such_pool(
    ...
    mockpatch(tft_artemis.drivers.SafeQuery, 'one_or_none').return_value = Ok(None)  # type: ignore[attr-defined]

    r_pool = tft_artemis.drivers.PoolDriver.load_or_none(logger, session, 'dummy-pool-that-does-not-exist')
...
def test_driver_load_or_none_db_error(
    ...
    mock_failure = tft_artemis.Failure('dummy failure')
    mockpatch(
        tft_artemis.drivers.SafeQuery,  # type: ignore[attr-defined]
        'one_or_none',
...
def test_driver_load_or_none_instantiate_error(
    ...
    mock_failure = tft_artemis.Failure('dummy failure')
    mockpatch(tft_artemis.drivers.PoolDriver, '_instantiate').return_value = Error(mock_failure)
...
def test_driver_load(
    ...
    mock_pool = MagicMock(name='pool<M>')
    mockpatch(tft_artemis.drivers.PoolDriver, 'load_or_none').return_value = Ok(mock_pool)

    r = tft_artemis.drivers.PoolDriver.load(logger, session, 'dummy-pool')
    ...
    cast(MagicMock, tft_artemis.drivers.PoolDriver.load_or_none).assert_called_once_with(logger, session, 'dummy-pool')


def test_driver_load_no_such_pool(
    logger: gluetool.log.ContextAdapter, session: sqlalchemy.orm.session.Session, mockpatch: MockPatcher
) -> None:
    mockpatch(tft_artemis.drivers.PoolDriver, 'load_or_none').return_value = Ok(None)
...
def test_driver_load_error(
    ...
    mockpatch(tft_artemis.drivers.PoolDriver, 'load_or_none').return_value = Error(tft_artemis.Failure('dummy failure'))
...
def test_generate_post_install_script_from_template_cloud_init(
    ...
    pool = tft_artemis.drivers.PoolDriver(
        logger,
...
def test_generate_post_install_script_from_template_default(
    ...
    pool = tft_artemis.drivers.PoolDriver(logger, 'dummy-driver-with-post-install-script', {})
    r_script = pool.generate_post_install_script(guest_request)
    assert r_script.is_ok
    script = r_script.unwrap()
