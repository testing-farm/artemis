...
# Flavor fields that are driver-internal and have no schema counterpart.
_FLAVOR_INTERNAL_TOP_LEVEL_FIELDS: set[str] = {'name', 'id'}

# hw_block properties that represent pool-specific hints rather than flavor attributes.
_HW_BLOCK_NON_FLAVOR_PROPS: set[str] = {'beaker', 'system'}


def test_latest_schema_hw_block_matches_flavor() -> None:
    from tft.artemis.api.environment import CURRENT_MILESTONE_VERSION
    ...
    hw_block_props = set(schema.get('definitions', {}).get('hw_block', {}).get('properties', {}).keys())
    hw_requirements_props = set(schema.get('definitions', {}).get('hw_requirements', {}).get('properties', {}).keys())
    schema_hw_props = hw_block_props | hw_requirements_props

    flavor_fields = (
        {field.name for field in dataclasses.fields(Flavor)}
        - _FLAVOR_INTERNAL_TOP_LEVEL_FIELDS
        - set(Flavor.VIRTUAL_PROPERTIES)
    )

    # Convert Python field names (underscore) to schema property names (hyphen).
    flavor_schema_names = {name.replace('_', '-') for name in flavor_fields}

    missing_from_schema = flavor_schema_names - schema_hw_props
    assert not missing_from_schema, (
        f'{CURRENT_MILESTONE_VERSION} schema missing HW properties defined in Flavor: {missing_from_schema}'
    )

    missing_from_flavor = (
        hw_block_props - flavor_schema_names - set(Flavor.VIRTUAL_PROPERTIES) - _HW_BLOCK_NON_FLAVOR_PROPS
    )
    assert not missing_from_flavor, (
        f'Flavor missing fields for `hw_block` properties in {CURRENT_MILESTONE_VERSION}: {missing_from_flavor}'
    )
