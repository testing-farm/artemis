...
def test_lock_sanity(logger: gluetool.log.ContextAdapter, cache: redis.Redis, lockname: str) -> None:
    token = tft_artemis.cache.acquire_lock(logger, cache, lockname)

    assert token is not None

    time.sleep(1)

    result = tft_artemis.cache.release_lock(logger, cache, lockname, token)
...
def test_lock_ttl_expired(
    ...
    token = tft_artemis.cache.acquire_lock(logger, cache, lockname, ttl=5)
...
def test_lock_ownership_change(
    ...
    tft_artemis.cache.set_cache_value(logger, cache, lockname, str(new_token).encode('utf-8'))
