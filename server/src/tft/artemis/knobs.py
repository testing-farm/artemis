...
class Knob(Generic[T]):
    ...
    * may be stored in a database.
    ...
    def __init__(
        ...
        # `value` attribute - no `Result`, no `unwrap()` - given the possible sources, it should never
        # fail to get a value from such sources.
        #
...
KNOB_GUEST_REQUEST_RESOURCE_DIGEST_TEMPLATE: Knob[str] = Knob(
    ...
    The rendered template is then parsed as a YAML, and must produce a mapping.
    """,
    has_db=False,
    per_entity=True,
