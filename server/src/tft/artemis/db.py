...
if TYPE_CHECKING:
    from . import Failure
    from .drivers import PoolResourceGuestRequestDigest
...
class GuestRequest(Base):
    ...
    def resource_digest(self) -> Result['PoolResourceGuestRequestDigest', 'Failure']:
        if self.poolname is None:
            return Error(
                Failure(
                    'cannot construct guest request resource digest for unknown pool',
        ...
        r_digest = render_template(
            r_template.unwrap(),
            **template_environment(self)
        )
