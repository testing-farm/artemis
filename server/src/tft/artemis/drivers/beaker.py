...
def constraint_to_beaker_filter(constraint: ConstraintBase) -> Result[bs4.BeautifulSoup, Failure]:
    ...
    if CONSTRAINT_DISK_PATTERN.match(constraint.name):
        ...
        # All other constraints can be ignored (for now) - they are not unsupported, they just don't matter.
...
def _prune_beaker_filter(tree: bs4.BeautifulSoup) -> Result[bs4.BeautifulSoup, Failure]:
    ...
    # Conversion process also produces grouping constraints that contain just a single item, resulting in `and`
    # and `or` with just a single child. Those elements can be replaced with their child.
