...
class FlasherDriver(PoolDriver):
    ...
    def acquire_guest(
        ...
        if not guest_id:
            return Error(Failure(
                'backend responded with no guest ID',
                payload=data
            ))
    ...
    def update_guest(
        ...
        pool_data = FlasherPoolData.unserialize(guest_request)
    ...
    def release_guest(
        ...
        if FlasherPoolData.is_empty(guest_request):
            return Ok(None)
    ...
    @guest_log_updater(self.drivername, 'flasher-debug:dump', GuestLogContentType.URL)  # type: ignore[arg-type]
    ...
    @guest_log_updater(self.drivername, 'flasher-event:dump', GuestLogContentType.URL)  # type: ignore[arg-type]
    ...
    @guest_log_updater(self.drivername, 'flasher-debug:dump', GuestLogContentType.BLOB)  # type: ignore[arg-type]
    ...
    @guest_log_updater(self.drivername, 'flasher-event:dump', GuestLogContentType.BLOB)  # type: ignore[arg-type]
    ...
    @guest_log_updater(self.drivername, 'console:dump', GuestLogContentType.BLOB)  # type: ignore[arg-type]
...
PoolDriver._drivers_registry[FlasherDriver.drivername] = FlasherDriver
