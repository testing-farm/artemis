...
from returns.result import Failure as _Error
from returns.result import Result as _Result
from returns.result import Success as _Ok
from tmt.hardware import UNITS
from typing_extensions import TypeAlias

from tft.artemis.drivers import PoolDriver, PoolImageInfo, PoolImageInfoT, create_tempfile
from tft.artemis.drivers.ibmcloud import IBMCloudDriver, IBMCloudFlavor, IBMCloudInstance, IBMCloudPoolData, \
    IBMCloudPoolResourcesIDs, IBMCloudSession, InstanceCreationOutcome, InstanceCreationRequest
...
from .. import GuestLogUpdateProgress, PoolCapabilities, PoolErrorCauses, PoolImageSSHInfo, ProvisioningProgress, \
    ProvisioningState, ReleasePoolResourcesState, SerializedPoolResourcesIDs, guest_log_updater
...
class IBMCloudPowerDriver(IBMCloudDriver[BackendInstance, IBMCloudPowerInstance, None]):
    ...
    def _query_backend_instance(
        ...
        with IBMCloudPowerSession(logger, self) as session:
            return self._query_backend_instance_reuse_session(session)

    # Note: the output of this method matches what `pi instance get` returns. In general, `instance get` and
    # `instance list` differ in the output structure significantly, this method may not fit all use cases it
    # fits in other drivers. Be careful when using it.
    def _query_backend_instance_reuse_session(
        self, logger: gluetool.log.ContextAdapter, instance_id: str, session: IBMCloudPowerSession
    ) -> _Result[BackendInstance, Failure]:
