...
def create_sanitize_tags(
    *,
    ...
    allowed_key_charset: Optional[str] = None,
    sanitize_key_charset: bool = True,
    sanitize_value_charset: bool = True,
    ...
    :param allowed_charset: A string of characters allowed in the tag value.
    :param allowed_key_charset: A string of characters allowed in the tag key. If ``None``, defaults to
        ``allowed_charset``.
    :param sanitize_key_charset: Wheter to apply character replacement to the key.
    :param sanitize_value_charset: Whether to apply character replacement to the value.
    ...
    allowed_key_charset = allowed_key_charset or allowed_charset

    def _create_substitution_fn(allowed_chars: str) -> Callable[[str], str]:
        ...
        def _map_val(val: str) -> str:
            return pattern.sub(_char_mapping, val)

        return _map_val

    def _noop(val: str) -> str:
        return val

    _map_key = _create_substitution_fn(allowed_key_charset) if sanitize_key_charset else _noop
    _map_value = _create_substitution_fn(allowed_charset) if sanitize_value_charset else _noop
...
class FlavorBasedPoolDriver(
    ...
    #: Name of the hook filtering suitable flavors.
    _flavor_filter_hook_name: str
    ...
    def _guest_request_to_flavor_or_none(
        ...
        r_engine = hook_engine(self._flavor_filter_hook_name)
        ...
        with Sentry.start_span(
            TracingOp.FUNCTION, description='run_hook', tags={'hookname': self._flavor_filter_hook_name}
        ):
            try:
                r_ruling: FilterReturnType[FlavorT] = engine.run_hook(
                    self._flavor_filter_hook_name,
                    logger=logger,
                    session=session,
                    pool=self,
                    guest_request=guest_request,
                    image=image,
                    flavors=r_suitable_flavors.unwrap(),
                )

            except Exception as exc:
                return Error(Failure.from_exc('unhandled hook error', exc))
