...
def create_guest(
    ...
    return create_guest_request(
        api_version='v0.0.69', guest_request=guest_request, manager=manager, request=request, auth=auth, logger=logger
    )
...
def get_guest_log(
    ...
    manager: Annotated[GuestRequestManager, Depends(GuestRequestManager)],
    logger: Annotated[gluetool.log.ContextAdapter, Depends(get_logger)],
) -> GuestLogResponse_v0_0_69:
    return get_guest_request_log(guestname, logname, contenttype, manager, logger, GuestLogResponse_v0_0_69)
