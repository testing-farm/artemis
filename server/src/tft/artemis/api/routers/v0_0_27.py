...
def create_guest(
    ...
    return create_guest_request(
        api_version='v0.0.27', guest_request=guest_request, manager=manager, request=request, auth=auth, logger=logger
    )
...
def refresh_pool_image_info(
    ...
    return CacheManager.entry_refresh_pool_image_info(
        poolname=poolname, manager=manager, logger=logger, request=request
    )
