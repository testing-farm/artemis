...
from . import ProvisioningTailHandler, TaskCall, get_guest_logger, step, task, task_core_v2
...
class Workspace(_Workspace):
    ...
    def run(self) -> None:
        with self.transaction():
            self.load_guest_request_v2(self.guestname, state=GuestState.PROVISIONING)
            ...
            if provisioning_progress.state == ProvisioningState.PENDING:
                ...
                if not ProvisioningTailHandler(GuestState.PROVISIONING, GuestState.SHELF_LOOKUP).handle_tail_v2(
                    self,
                    TaskCall(
