...
from . import ProvisioningTailHandler, get_guest_logger, resolve_actor, step, task, task_core_v2
...
class Workspace(_Workspace):
    ...
    def run(self) -> None:
        with self.transaction():
            self.load_guest_request_v2(self.guestname, state=GuestState.PREPARING)
            self.mark_note_poolname()
            self.load_gr_pool_v2()
            ...
            self.update_guest_state_and_request_task_v2(
                GuestState.READY,
            ...
            self.load_guest_events_v2(eventname='error')
            ...
            for taskname, arguments in self.gr.on_ready:
                ...
                self.request_task_v2(r_actor.unwrap(), self.guestname, *arguments)
