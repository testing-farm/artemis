...
from .. import Failure
from ..db import DB, GuestLogContentType, GuestLogState, execute_dml
from ..drivers import GuestLogBlob, GuestLogUpdateProgress, copy_from_remote, guest_log_update_log, ping_shell_remote, \
    run_remote
            ...
            def _pull_log(log_type: AnacondaLogType, finished: bool = False) -> None:
                ...
                with tempfile.TemporaryDirectory() as tmpdir:
                    ...
                    with open(dst) as f:
                        ...
                        r_update_log = guest_log_update_log(
                            self.logger,
                            self.session,
                            self.gr.guestname,
                            f'{log_type.value}.log:dump',
                            GuestLogContentType.BLOB,
                            GuestLogUpdateProgress(
                                state=GuestLogState.COMPLETE,
                                overwrite=True,
                                blobs=[GuestLogBlob.from_content(log)]
                            ),
                            create=True
                        )

                        if r_update_log.is_error:
                            self._error(r_update_log, f'failed to update {log_type.value}.log', no_effect=True)
