...
# post container exit
ExecStart=/usr/bin/podman run --privileged -d -v /var/log:/var/log/host:ro {%- for path in contained_harness_ro_host_volumes|default('/etc/localtime')|split(',') %} -v {{ path }}:{{ path }}:ro{% endfor %}{%- for path in contained_harness_rw_host_volumes|default('/mnt,/root')|split(',') %} -v {{ path }}:{{ path }}:rw{% endfor %} --name beaker-harness -t beaker-harness
User=root
Group=root
TimeoutStartSec=0
