...
cp /etc/yum.repos.d/rhel.repo /root/
...
cat <<"__DOCKERFILE_EOF__" > /root/Dockerfile
...
RUN cat <<"EOF" >/etc/yum.repos.d/{{ reponame }}.repo
[{{ reponame }}]
name={{ reponame }}
baseurl={{ repourl }}
enabled=1
...
EOF
{% endif %}
COPY beaker-tasks.repo /etc/yum.repos.d/
{% if customrepos %}
COPY customrepos/ /etc/yum.repos.d/
{% endif %}
...
# Install the harness
RUN dnf -y install {{ harness|default('restraint restraint-rhts') }}
RUN dnf -y --allowerasing install coreutils
RUN dnf -y install beakerlib || true
RUN dnf -y install beakerlib-redhat || true
...
ExecStartPre=/usr/bin/podman build -t beaker-harness .
# Mount the host /mnt at /mnt so that the test data is preserved
# post container exit
ExecStart=/usr/bin/podman run --privileged -d {%- for path in contained_harness_ro_host_volumes|default('/etc/localtime')|split(',') %} -v {{ path }}:{{ path }}:ro{% endfor %}{%- for path in contained_harness_rw_host_volumes|default('/mnt,/root')|split(',') %} -v {{ path }}:{{ path }}:rw{% endfor %} --name beaker-harness -t beaker-harness
User=root
Group=root
TimeoutStartSec=0
