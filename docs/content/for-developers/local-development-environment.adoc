...
....
+
* edit this new `env.yml` file and provide details of your OpenStack tenant and an SSH key pair to be used by Artemis as its master-key.
* create an SSH keypair for Artemis, and upload it to your OpenStack tenant matching the `openstack.vms.keyname` in your `env.yml`. For example, if you set `openstack.vms.keyname` to `artemis`:
+
[source,shell]
....
