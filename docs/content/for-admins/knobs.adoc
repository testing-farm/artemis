...
|`str`
|`ARTEMIS_API_ENGINE_BIND_ADDRESS`
|`0.0.0.0`
|no



5+|Address to bind the listening socket to.

|`api.engine.bind.port`
|`int`
|`ARTEMIS_API_ENGINE_BIND_PORT`
|`8001`
|no



5+|Port to bind the listening socket to.
...
|`str`
|`ARTEMIS_INSTANCE_FLAVOR_TAG`
|`ArtemisFlavor`
|no


|`artemis.instance.flavor.tag:$entityname`
|
|`ARTEMIS_INSTANCE_FLAVOR_TAG_$entityname`
|
|


5+|Artemis tag to hold the actual instance flavor

|`artemis.instance.guest-label.tag`
|`str`
|`ARTEMIS_INSTANCE_GUEST_LABEL_TAG`
|`ArtemisGuestLabel`
|no


|`artemis.instance.guest-label.tag:$entityname`
|
|`ARTEMIS_INSTANCE_GUEST_LABEL_TAG_$entityname`
|
|


5+|Artemis tag to hold the human-readable guest label, usable e.g. for naming instances. Changes over time.

|`artemis.instance.guest-name.tag`
|`str`
|`ARTEMIS_INSTANCE_GUEST_NAME_TAG`
|`ArtemisGuestName`
|no


|`artemis.instance.guest-name.tag:$entityname`
|
|`ARTEMIS_INSTANCE_GUEST_NAME_TAG_$entityname`
|
|


5+|Artemis tag to hold the guestname identifying the guest request. Does not change over time.

|`artemis.mapping.instance-name.template`
|`str`
|`ARTEMIS_INSTANCE_NAME_TEMPLATE`
|`artemis-{{ GUESTNAME }}`
|no


|`artemis.mapping.instance-name.template:$entityname`
|
|`ARTEMIS_INSTANCE_NAME_TEMPLATE_$entityname`
|
|
...
|`bool`
|`ARTEMIS_DISPATCHER_USE_LOWER_ISOLATION_LEVEL`
|`no`
|no



5+|If set, a lower isolation level, ``READ COMMITTED``, would be used for dispatcher DB sessions instead of the default
...
|`int`
|`ARTEMIS_POOL_RESOURCES_PER_REQUEST_LIMIT`
|`7`
|no


|`pool.resources-per-request-limit:$entityname`
|
|`ARTEMIS_POOL_RESOURCES_PER_REQUEST_LIMIT_$entityname`
|
|


5+|Maximum number of resources allowed to be present in cloud simultaneously for a single guest request
...
|`int`
|`ARTEMIS_SENTRY_MAX_VALUE_LENGTH`
|`102400`
|no



5+|The number of characters after which the values containing text in the event payload will be truncated.

|`sentry.tracing.sample-pattern`
|`str`
|`ARTEMIS_SENTRY_TRACING_SAMPLE_PATTERN`
|`.*`
|no



5+|Only tasks matching this pattern would be reporting their tracing data to Sentry.
...
|`str`
|`ARTEMIS_WORKER_HEARTBEAT_FILEPATH_TEMPLATE`
|`/var/run/artemis/worker-heartbeat-{{ WORKER_PID }}-{{ WORKER_TID }}`
|no



5+|Template of the heartbeat file path.
