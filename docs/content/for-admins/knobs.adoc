...
|`str`
|`ARTEMIS_BEAKER_RESERVATION_EXTENSION_CONTAINER_COMMAND_TEMPLATE`
|`podman exec beaker-harness /bin/bash -c "echo {{ (EXTENSION_TIME / 3600) | int }} | extendtesttime.sh"`
|no



5+|A template for a command to run to extend Beaker reservation with containerized harness.
