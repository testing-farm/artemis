...
* `ARTEMIS_DB_POOL_MAX_OVERFLOW` - Maximum size of connection pool overflow, by default `10`
...
* `ARTEMIS_LOG_LEVEL` (choice): Level of logging. Accepted values are Python logging levels as defined by Python's https://docs.python.org/3.7/library/logging.html#levels[logging subsystem]. Default: `info`.
* `ARTEMIS_VAULT_PASSWORD_FILE` (string): Path to a file with a Vault password for decrypting protected files. Default: `$HOME/.vault_password`.
...
* `ARTEMIS_ROUTE_POOL_RESOURCE_THRESHOLD` (float): A percentage part of pool resource that, when reached, marks pool as depleted and not eligible for provisioning. Default: `90`.
