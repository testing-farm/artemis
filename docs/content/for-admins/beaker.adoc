...
By default, the driver talks to the Beaker hub configured in the environment, e.g. via `/etc/beaker/client.conf`. To point a pool to a different Beaker server, specify its hub URL.
...
[source,yaml]
....
hub: https://another-beaker.example.com
....

[TIP]
====
Combined with `username` and `password`, this allows different pools to use different Beaker servers, each with its own credentials.
====
...
[NOTE]
====
Beaker driver is currently the only driver providing additional logs next to console logs.
====
...
|`console:dump`
|URL
|URL pointing to the stored console output.

|`sys.log:dump`
|URL
|Contains low-level system messages generated during the installation process.

|`anaconda.log:dump`
|blob
|Anaconda installer application log.

|`storage.log:dump`
|blob
|Anaconda disk-related subsystem log.

|`program.log:dump`
|blob
|Records every external command Anaconda runs with output and exit codes.

|`packaging.log:dump`
|blob
|Logs all package installation actions done by Anaconda.

|`ks.cfg:dump`
|blob
|Kickstart configuration Anaconda generated.
