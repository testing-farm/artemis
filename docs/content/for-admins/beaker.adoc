...
Beaker driver provides console log access through stored console output. For details on console support across all drivers, see <<drivers.adoc#_console_support_details,"Console support details">>.
...
|`console:dump`
|blob
|Console output stored by Beaker

|`console:dump`
|URL
|URL pointing to Beaker's stored console output

|`sys.log:dump`
|URL
|URL pointing to system log stored by Beaker
...
|`storage.log:dump`
|blob
|Storage configuration log from installation

|`program.log:dump`
|blob
|Program execution log from installation
...
|`ks.cfg:dump`
|blob
|Kickstart configuration file used for installation

|===

==== Interactive console via conserver

Beaker provides interactive console access via conserver. This is available directly through Beaker, external to Artemis. Users can connect to conserver directly to access the live console of their provisioned machines.

[NOTE]
====
Interactive console via conserver is not exposed through Artemis API. Users need to connect to conserver directly using the appropriate Beaker credentials and machine hostname.
====
